...
import dev.langchain4j.agentic.CognisphereAccess;
import dev.langchain4j.agentic.ResultWithCognisphere;
...
public abstract class AbstractAgentInvocationHandler implements CognisphereOwner {
    ...
    private boolean rootForSingleCallCognisphere = false;
    ...
    private Object executeAgentMethod(Cognisphere cognisphere, Method method, Object[] args) {
        ...
        if (rootForSingleCallCognisphere) {
            Cognisphere.registry().evict(cognisphere.id());
            rootForSingleCallCognisphere = false;
        }
    ...
    private Cognisphere currentCognisphere(Method method, Object[] args) {
        ...
        Cognisphere singleCallCognisphere = new Cognisphere();
        Cognisphere.registry().register(singleCallCognisphere);
        rootForSingleCallCognisphere = true;
        return singleCallCognisphere;
    }
